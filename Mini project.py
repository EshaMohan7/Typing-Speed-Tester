...
def getvals():
    ...
    def speed(stime, etime, inprompt):
        ...
        labelfortimer = Label(root, text='Your speed (wpm):', font=(
            'arial', 20, 'italic bold'), fg='red')
        labelfortimer.place(x=300, y=300)
        timercount = Label(root, text=tspeed, font=(
            'arial', 20, 'italic bold'), fg='blue')
    ...
    def error(prompt, inprompt):
        ...
        for i in range(min(len(inwords), len(words))):
            try:
                ...
                errors += 1
        labelerror = Label(root, text='Total errors:', font=(
            'arial', 20, 'italic bold'), fg='red')
        labelerror.place(x=300, y=400)
        errorcount = Label(root, text=errors, font=(
            'arial', 20, 'italic bold'), fg='blue')
        ...
        labelacc = Label(root, text='Your accuracy:', font=(
            'arial', 20, 'italic bold'), fg='red')
        labelacc.place(x=300, y=500)
        acount = Label(root, text=round(acc, 2), font=(
            'arial', 20, 'italic bold'), fg='blue')
        acount.place(x=300, y=550)
        percent = Label(root, text="%", font=(
            'arial', 20, 'italic bold'), fg='blue')
    ...
    print(
        f"Speed: {round(speed(stime,etime,inprompt),2)} wps (words per second)")
    print(f"Error: {round(error(prompt,inprompt),2)}")
    print("\n############################\n")


name = Label(text="Check your typing speed NOW!", bg="Yellow",
             font="Arial 30 bold", pady=30, padx=20)
name.grid()
...
htext = Label(root, text="(Type the above text)",
              font="Times_new_roman 12 ", pady=10, padx=10)
htext.grid()
...
userentry = Entry(root, textvariable=uservalue,
                  width=90, font="Times_new_roman 12 ")
